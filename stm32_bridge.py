...
def print_tof_matrix(frame_cnt, distances, show_matrix):
    if show_matrix:
...
def process_payload(msg_type, payload, show_matrix):
    ...
    if msg_type == 0x82 and len(payload) >= 132:
        ...
        print_tof_matrix(frame_cnt, distances, show_matrix)


def process_buffer(buffer, show_matrix):
    ...
    while len(buffer) >= 7:
        ...
        process_payload(msg_type, payload, show_matrix)
...
def main():
    ...
    while True:
        try:
            ...
            process_buffer(buffer, args.show_matrix)
